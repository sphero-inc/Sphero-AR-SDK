...
@interface RKDeviceMessenger : NSObject {
    ...
    uint64_t            currentDataStreamingMask;
    int                 messageRateDivisor;
    bool                isStreamingData;
}

/* Whether DeviceMessenger is controling data streaming */
@property (nonatomic, readonly) bool isStreamingData;
 ...
 * @param observer The observer. Only one selector per observer is allowed.
 * @param handler A selector of the form - (void)handDataStreaming:(RKDeviceAsyncData *)data
 * @param mask is the mask of values that the observer wants to receive
 */
- (void)addDataStreamingObserver:(id)observer selector:(SEL)handler mask:(uint64_t)mask;
...
/*!
 * Get the current data streaming mask being managed by the Device Messenger
 */
-(uint64_t)getCurrentDataStreamingMask;

/*!
 * Set the current message rate divisor only if streaming is already on
 */
-(void)setMessageRateDivisor:(int)divisor;
